...
void draw_points(MLX *mlx) {
    ...
    while (i < 4) {
        ...
        // Vérification des coordonnées avant affichage
        if (projected.x >= 0 && projected.x < WIN_WIDTH &&
            projected.y >= 0 && projected.y < WIN_HEIGHT) {
...
int main() {
    ...
    // Dessiner les points une fois avant la boucle
    draw_points(&mlx);
